(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    _.each(array, function(value) {
      if(!uniqValues.hasOwnProperty(iterator(value))) {
        uniqValues[iterator(value)] = value
      }
      
    })
    for (var key in uniqValues) {
      processedArray.push(uniqValues[key]);
    }
  ...
  _.reduce = function(collection, iterator, accumulator) {
    if (Array.isArray(collection)) {
      ...
      for (var i = 0; i < newCollection.length; i++) {
        accumulator = iterator(accumulator, newCollection[i]);
      }
    } else if (typeof collection === 'object') {
      var keyArray = Object.keys(collection);

      for (var i = 0; i < keyArray.length; i++) {
        accumulator = iterator(accumulator, collection[keyArray[i]]);
      }
